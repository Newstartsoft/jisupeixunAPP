...
+function ($) {
  "use strict";
    ...
    //新增了一个参与，可设置文本框默认值
    $.prompt = function (text, title, callbackOk, callbackCancel,value) {
        if (typeof title === 'function') {
            callbackCancel = arguments[2];
            callbackOk = arguments[1];
            title = undefined;
        ...
        return $.modal({
            text: text || '',
            title: typeof title === 'undefined' ? defaults.modalTitle : title,
            afterText: '<input type="text" class="modal-text-input" value='+value+'  autofocus="autofocus" />',
            buttons: [
                {
